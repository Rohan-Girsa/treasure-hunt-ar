 ...
 * Treasure placed 5m in random cardinal direction from base coordinates
 *************************************************/
...
// Global state
let treasureLocation = null; // Will be set to 5m in random direction
let activeEntity = null;
let isCaptured = false;
let isCapturing = false; // NEW: Prevent multiple simultaneous captures
let sceneEl = null;
...
// FIXED SINGLE GLB
const TREASURE_CHEST_URL = "./assets/treasure-chest/tc-1.glb";
const TREASURE_CHEST_SCALE = [0.8, 0.8, 0.8];
const CHEST_ANIMATION_INDEX = 0;
const CHEST_ANIMATION_START_TIME = 0.75;
...
AFRAME.registerComponent("chest-animation", {
  ...
  tick: function () {
    ...
    // FIXED: Only rotate when not animating - chest stays at GPS location
    if (this.model && !this.isAnimating && this.modelLoaded) {
  ...
  completeCapture: function () {
    ...
    if (messageEl) {
      messageEl.innerHTML = `${modelName} Captured!<div style="font-size: 10px; margin-top: 12px; font-weight: 500;">Treasure is now yours!</div>`;
  ...
  onModelLoaded: function (model) {
    ...
    // FIXED: Remove the position locking - let GPS handle positioning
    ...
    glowMesh.position.set(0, 0.85, 0);
...
window.onload = () => {
  ...
  // FIXED: Random placement in cardinal directions (N, S, E, W)
  // 0° = North, 90° = East, 180° = South, 270° = West
  const directions = [0, 90, 180, 270];
  const randomBearing = directions[Math.floor(Math.random() * directions.length)];
  const distance = 5; // 5 meters away
  
  treasureLocation = offsetLatLng(BASE_LATITUDE, BASE_LONGITUDE, distance, randomBearing);

  console.log("Base coordinates:", BASE_LATITUDE, BASE_LONGITUDE);
  console.log(`Treasure location (${distance}m at ${randomBearing}°):`, treasureLocation);
...
function tryCapture(event) {
  // FIXED: Check multiple conditions to prevent duplicate captures
  if (
    event.target.tagName === "BUTTON" || 
    !activeEntity || 
    isCaptured || 
    isCapturing // NEW: Prevent clicking while animation is running
  ) {
    return;
  }
  ...
  if (distance <= CAPTURE_DISTANCE) {
    ...
    isCapturing = true; // FIXED: Set flag immediately to prevent double-clicks
